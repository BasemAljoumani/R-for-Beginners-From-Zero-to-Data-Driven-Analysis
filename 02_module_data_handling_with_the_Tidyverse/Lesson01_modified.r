...
df_csv <- read_csv("EC_teaching.csv")
...
df_dta <- read_stata("EC_teaching.dta")
...
df <- readRDS("EC_teaching.rds")

dim(df)



#url(rul2) → this is an internet connection, not a local filename
#open = "rb" → open it in read-binary mode
#readRDS() → deserialize the R object coming through that connection
...
rul2 <- "https://raw.githubusercontent.com/BasemAljoumani/R-for-Beginners-From-Zero-to-Data-Driven-Analysis/main/02_module_data_handling_with_the_Tidyverse/data/EC_teaching.rds"

df2 <- readRDS(url(rul2, open = "rb"))
...
dim(df2)     # [1] 8759   12
nrow(df2)    # 8759 rows = 8759 hours of measurements
ncol(df2)    # 12 columns = 12 variables
...
#   <chr>      <dbl>    <dbl>      <dbl>
#1 Autumn      10.9     29.3      136. 
#2 Spring      10.3     25.4       92.5
#3 Summer      20.9     37.8      126. 
#4 Winter       3.4     16.3       89.
...
# A tibble: 0 × 12summary(df)
# Correct base R approach
